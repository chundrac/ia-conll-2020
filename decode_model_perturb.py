...
ind = 0
if 'decoded_perturbed_{}_{}_{}.tsv'.format(seed,k,mode) in os.listdir('.'):
    ind = len([l for l in open('decoded_perturbed_{}_{}_{}.tsv'.format(seed,k,mode),'r')])

for l,p,s,i,o in zip(lang_raw['test'][ind:],POS_raw['test'][ind:],gloss_raw['test'][ind:],input_raw['test'][ind:],output_raw['test'][ind:]):
    f = open('decoded_perturbed_{}_{}_{}.tsv'.format(seed,k,mode),'a')
    if p == 'V':
        p_ = 'N'
    if p == 'N':
        p_ = 'V'
    o_ = decode_sequence(i,l,p_,s,langs,output_segs,input_segs,POS_segs,L,X,Y,J,S,T_x,T_y,model,mode)
    d = editdistance.distance(o,o_)/max([len(o),len(o_)])
    print('-')
    print('\t'.join([l,p,''.join(i),''.join(o),''.join(o_),str(d)]),file=f)
    f.close()
